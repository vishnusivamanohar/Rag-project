...
def check_history(query, history):
    query_words = set(re.findall(r'\w+', query.lower()))
    stop_words = {
        'is', 'now', 'the', 'restaurant', 'opened', 'open', 'close', 'closed',
        'are', 'and', 'or', 'a', 'an', 'to', 'in', 'of', 'for', 'on', 'with', 
    ...
    if keywords and len(history) > 2:
        for msg in history[:-1]:
            if msg.get("role") == "user":
                msg_content = msg.get("content", "").lower()
                msg_words = set(re.findall(r'\w+', msg_content))
                if keywords.issubset(msg_words):
...
def get_bot_response(query, history):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        future_history = executor.submit(check_history, query, history)
        ...
        already_in_history, matched_prev_query = future_history.result()
    ...
    # 3. History Search Status
    print("\n\033[92m[3. History Search] Status:")
    if already_in_history:
        print(f"  Match Found! Similar query asked earlier: '{matched_prev_query}'")
        print("  -> Decision: Answer will be generated directly from Chat History.")
    else:
        print("  No Match Found in history.")
    print("\033[0m" + "=" * 60 + "\n")
    ...
    if already_in_history:
        all_chunks = []
        context = ""
        print("\033[92m[Response Mode] Answer taken from history (RAG retrieval skipped for LLM context).\033[0m\n")
    else:
